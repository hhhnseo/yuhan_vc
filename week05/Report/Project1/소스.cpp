...
// 그림 그리기 시작점
POINT startPoint;
BOOL isDrawing = FALSE; // 그림 그리기 모드
BOOL isMoving = FALSE;  // 그림 이동 모드
HWND hwnd;              // 윈도우 핸들
HBRUSH drawingBrush, whiteBrush;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (uMsg == WM_CREATE) {
        ...
        drawingBrush = CreateSolidBrush(RGB(255, 0, 100));     // 브러쉬 생성
        isMoving = FALSE;
        isDrawing = FALSE;
        moveOffsetX = 0;
        moveOffsetY = 0;
...
#endif
{
    ...
    HWND hwnd = CreateWindow(
        wc.lpszClassName,
        TEXT("202207032 JangHyeonseo"), // 윈도우 제목
